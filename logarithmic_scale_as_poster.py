...
file_paths = {
    "3dpf": r"C:\Users\Laris\OneDrive\Desktop\New_csvfiles_newdata\Dopamine_1stcon\Dopamine_3DPF_1STCON_temp.vtr 3dpf 21 oct 2024 dopamine 1st con.csv",
    "4dpf": r"C:\Users\Laris\OneDrive\Desktop\New_csvfiles_newdata\Dopamine_1stcon\Dopamine_4dpf_1st_con.csv",
    "5dpf": r"C:\Users\Laris\OneDrive\Desktop\New_csvfiles_newdata\Dopamine_1stcon\Dopamine_5dpf_dopamine_1stcon.csv"
}
...
def plot_combined_blocked(df):
    ...
    for i, param in enumerate(parameters):
        ...
        ax.set_ylabel(param + unit, fontsize=20, fontweight='bold')
        ax.set_xticks(tick_positions)
        ax.set_xticklabels(tick_labels, fontsize=18, fontweight='bold')
        ...
        for age in age_order:
            for mode in mode_order:
                ...
                if pval < 0.05:
                    ...
                    ax.text((start + end)/2, y + 0.5, sig, ha='center', va='bottom', fontsize=18)

    for ax in axs:
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        
    # Get the drug name for the legend label (capitalized for presentation)
    drug_title = df["drugname"].iloc[0].capitalize()
        
    # Legend for swim mode and treatment
    legend_elements = [
        Patch(facecolor='#AAAAAA', edgecolor='black', label='Control'),
        Patch(facecolor='#1267E9', edgecolor='black', label= f'Slow swim– {drug_title}'),
        Patch(facecolor='#D62728', edgecolor='black', label= f'Fast swim– {drug_title}')
        
    ]
    
    # Place legend *inside* the graph, but high enough to not cover the violins
    axs[0].legend(
        handles=legend_elements,
        loc='upper right',
        bbox_to_anchor=(0.98, 2),  # ← pushes it up without leaving figure space
        frameon=False,
        fontsize=14,
        title='Legend',
        title_fontsize=14,
    ...
    plt.subplots_adjust(hspace=0.4, top=0.73)  # Reserve space for legend at the top
